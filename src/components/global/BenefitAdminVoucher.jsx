...
const BenefitAdminVoucher = ({ open, handleClose,userData, role }) => {
  ...
  const [withinLimit, setWithinLimit] = useState(null); 
   ...
   useEffect(() => {
  const handle = async () => {
    const staff = userData?.staffWithAirtimeAllocation;
    ...
    if (Array.isArray(staff) && staff.length > 0) {
      ...
      console.warn("No staffWithAirtimeAllocation data found");
    }
  };
...
  }, [userData]);
  ...
  useEffect(() => {
    if (userData) {
      const updatedRows = rows.map((row) => {
        switch (row.id) {
          case 9:
            return { ...row, column2:userData.staffWithAirtimeAllocation[0].FullName };
          case 10:
          ...
          case 12:
            return { ...row, column2:userData.staffWithAirtimeAllocation[0].AirtimeAllocation };
          case 13:
  ...
  const handleInputChange = (event, id, field) => {
    ...
    if (field === "column2") {
      ...
      // Call calculateMUL with updated devicePrices
      const updatedRows = calculateMUL(rows, devicePrices);

      // Update state or context with updatedRows if needed
      setRows(updatedRows);
    ...
    if (row.id >= 1 && row.id <= 8) {
      ...
      if (row.id >= 6 && row.id <= 8) {
        return (
          ...
          parseFloat(devicePriceRefCol6To8.current && devicePriceRefCol6To8.current[row.id] ? devicePriceRefCol6To8.current[row.id].value || 0 : 0)
        );
      }
  ...
  try {
    ...
    for (const packageRow of packageRows) {
      ...
      if (packageRow.column2) {
        ...
        if (!packageRow.column6 || packageRow.column6 === "Select Type") {
          handleClose();
          const packageName = packageRow.dropdown && packageRow.dropdown !== "Select Package"
    ...
    if (selectedPackagesDetails.length === 0) {
      handleClose();
      throw new Error("Please select at least one package from the first five rows.");
    }
    ...
    const deviceRefs = [deviceNameRef.current[6], deviceNameRef.current[7], deviceNameRef.current[8]];
    const devicePriceRefs = [devicePriceRef.current[6], devicePriceRef.current[7], devicePriceRef.current[8]];
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (deviceName || (!isNaN(devicePrice) && devicePrice > 0)) {
        if (isNaN(devicePrice) || devicePrice <= 0) {
          handleClose();
          throw new Error(`Device price for ${deviceName || `device in row ${deviceIdx}`} must be a positive number.`);
        }
        selectedDevicesDetails.push({
          ...
          UpfrontPayment: parseFloat(upfrontPaymentRef.current[deviceIdx]?.value) || 0, // Assuming upfront for each device is in its row's ref
        });
      }
    }
    ...
    if (!employeeCode) {
      handleClose();
      throw new Error("Employee code is missing. Please enter an employee code.");
    }

    let msisdn = "";
    if (role === 1) { // Admin role check for MSISDN
      msisdn = rows.find((row) => row.id === 11)?.column2 ||
               rows.find((row) => row.id === 11)?.column3;
      if (!msisdn) {
        handleClose();
        throw new Error("MSISDN is missing. Admin must fill this field.");
      }
      // If msisdnRef.current[6] is intended for MSISDN, then the above row.find is redundant.
      // Assuming row 11 is the definitive source for admin-entered MSISDN.
      // If msisdnRef.current[6] is some other field, it should not overwrite msisdn here.
    }
    // If MSISDN can also be entered by a regular user (role 3) and it's in msisdnRef.current[6],
    // then uncomment/adjust this line, but it might conflict with the admin logic if both apply.
    // msisdn = msisdnRef.current[6]?.value || "";


    // --- 7. Recalculate MUL and Finalize Payments ---
    // Ensure `devicePriceRefCol6To8` is updated for `calculateMUL` if it depends on specific device refs for row 6-8
    // I've removed the direct assignments to devicePriceRefCol6To8.current[6/7/8] here, assuming
    // calculateMUL will correctly pick up values from the `rows` state or its own internal refs.
    // If calculateMUL specifically needs these refs to be updated *before* it runs, re-add them.

    const updatedRows = calculateMUL(rows); // `rows` should contain the latest user inputs
    const currentAllowance = parseFloat(updatedRows.find((row) => row.id === 12)?.column2 || 0);
    const newAllowance = parseFloat(updatedRows.find((row) => row.id === 14)?.column2 || 0); // Row 14 for New Allowance
    const limitCheckStatus = updatedRows.find((row) => row.id === 14)?.column5; // Row 14 for Limit Check Status

    // Calculate total monthly payment across all selected packages + currentAllowance - newAllowance
    const totalPackagesMonthlyCost = selectedPackagesDetails.reduce((sum, pkg) => sum + pkg.AdjustedMonthlyPrice, 0);
    // The previous `monthlyPayment` was `currentAllowance - newAllowance`.
    // It's unclear if `totalPackagesMonthlyCost` replaces this or adds to it.
    // Assuming `monthlyPayment` represents the *difference* in allowance *after* package costs are considered.
    // You might need to clarify how `totalPackagesMonthlyCost` interacts with `currentAllowance` and `newAllowance`.
    // For now, I'll keep the original `monthlyPayment` calculation and ensure `Packages` array carries individual costs.
    ...
    const contractData = {
      ...
      ApprovalStatus: role === 1 ? "Approved" : "Pending",
      ...
      // Global Device details (if they represent a primary device or total summary)
      // This is less clear how global deviceName/Price/Upfront relates to individual assigned devices.
      // You might want to remove these if device details are entirely per package in the `Packages` array.
      // DeviceName: deviceName,
      // DevicePrice: devicePrice, // This would be the price of device from row 6 only if `devicePriceRef.current[6]`
  ...
  text: "Your application has been successfully received.",
      ...
      try {
        const pendingContractResponse = await axiosInstance.get(
          `/contracts/latestPendingEmployeeContract/${employeeCode}`
        );
        if (pendingContractResponse.status === 201 || pendingContractResponse.status === 200) {
              
            }
      } catch (error) {
        console.error("Error fetching or updating contract:", error);
        Swal.fire({
          icon: "error",
          title: "API Error",
          text: "Failed to fetch or update contract. Please try again.",
        });
      }
    } else {
      throw new Error("Unauthorized role. Only admins and users can save contracts.");
    }
  } catch (error) {
    handleClose();
  ...
  const mapContractDataToDataGrid = (contractData) => {
    return rows.map((row) => {
      switch (row.id) {
        ...
        case 9:
          return { ...row, column2:updatingData.staffWithAirtimeAllocation[0].FullName };
        case 10:
        ...
        case 13:
          return { ...row, column2:updatingData.staffWithAirtimeAllocation[0].AirtimeAllocation };
        case 14:
          return {
            ...row,
          ...
          <div className="top text-center">
            ...
            <div className="mt-4 row justify-content-end">
              <div className="col-4">
                ...
                >
                  Upload Contract
                  <UploadFileIcon size={16} style={{ marginLeft: "10px" }} />
                </Button>
              </div>
            ...
            {role === 1 ? (
              <div>
                <p className="mt-5 w-75 mx-auto">
                  I understand that the Package(s) are, and remain my property
                  and that the contract entered into with MTC is legally
                  binding. Part of my cellphone airtime benefit is applied to
                  pay my package subscription fees while in the employ of MTC.
                  In the event of termination of employment and/or in the event
                  of delayed roaming charges, I am payment responsible for my
                  Package(s) and the balance may be deducted from my last
                  salary. All other terms and conditions apply as per the MTC
                  Cellphone and Equipment Benefit policy and the MTC Subscriber
                  Agreement.
                </p>
                <ul className="list-inline p-5">
                  <li className="list-inline-item col-md-6">
                    Employee Confirmation / Signature
                    .....................................................
                  </li>
                  <li className="list-inline-item border-bottom border-light-subtle col-md-4">
                    Date: {currentDate}{" "}
                  </li>
                </ul>

                <div className="table-responsive">
                  <table className="table table-md table-striped table-bordered w-75 mx-auto">
                    <thead className="thead-light">
                      <tr>
                        <td scope="row">HR Admin:</td>
                        <td scope="row" style={{ width: "250px" }}>
                          Johanna Kavendji
                        </td>
                        <td scope="row">Signature:</td>
                        <td scope="row" style={{ width: "200px" }}></td>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td scope="row">MH Supervisor:</td>
                        <td scope="row" style={{ width: "250px" }}>
                          Eric Williams
                        </td>
                        <td scope="row">Signature:</td>
                      ...
                      <tr>
                        <td scope="row">Key Account Executive:</td>
                        <td scope="row" style={{ width: "250px" }}>
                          Theo Ishuna
                        </td>
                        <td scope="row">Signature:</td>
                ...
                <div className="important text-center">
                  <h3 className="mt-4">IMPORTANT!!</h3>
                  <ul>
                    <li className="">
                      AR MUST return signed off voucher to HR:Admin.
                    </li>
                    <li>
                      Each control point (AR & KA) must scan signed voucher to
                      order.
                    </li>
                    <li>
                      For all excess/ rebate payments on more expensive devices,
                      there must be a receipt that must be scanned and loaded on
                      Siebel
                    </li>
                  </ul>
                </div>
...
export default BenefitAdminVoucher;
